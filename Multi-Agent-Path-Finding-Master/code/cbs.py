...
import heapq
import random
from single_agent_planner import compute_heuristics, space_time_a_star, get_location, get_sum_of_cost
...
def detect_collision(path1, path2):
    ...
    for t in range (0,timestep_total):
        # Edge collision
        # initial edge (in planned path) collision check starting from t=1, i.e. edges consisted by (t=0, t=1)
        if t!=0 and get_location(path1,t-1) == get_location(path2,t) and get_location(path1,t) == get_location(path2,t-1):
            ...
            # edge collision is constructed by agent2 [t-1,t] time-step location, correspondingly agent1's collision is [t,t-1]
            # return collision  # would return the first collision in (path1, path2)?
        # Vertex collision
        # which can be initiated at t=0
...
class CBSSolver(object):
    """CBS: two level search
            high level--resolves collisions by adding constraints
            low level--compute paths consistent with those constraints. """
    ...
    def find_solution(self, disjoint=True):
        ...
        for i in range(self.num_of_agents):  # Find initial path for each agent
            path = space_time_a_star(self.my_map, self.starts[i], self.goals[i], self.heuristics[i],
                          i, root['constraints'])
            if path is None:
        ...
        while len(self.open_list) > 0:
            ...
            # if current node has no collisions in cbs priority queue, return the solution paths for each agent
            ...
            collision = new_node['collisions'][0]   # the first element in collisions of current pop node
            ...
            for constraint in constraints:
                ...
                Q={'constraints' : [],
                    ...
                    'cost' : [],
                    'collisions' : []}
                # 'new_node' is the parent node with detected collision, which has already been used for generated constraints and will be added into its child's nodes who inherits the parent constraints
                ...
                # since we only need to update the conflicted agents' paths, thus firstly copy all the paths in the parent's node to maintain the unchanged agents' paths
                for i in new_node['paths']:

                    Q['paths'].append(i)

                # only update the new constraint corresponding agent's path
                agent = constraint['agent']
            
                path = space_time_a_star(self.my_map, self.starts[agent], self.goals[agent], self.heuristics[agent],agent, Q['constraints'])

                if path is not None:
            
                    # raise BaseException('No solutions')

                    Q['paths'][agent] = path    # only update the new constraint indexed agent's path

                    # initial paths_violate should be an empty list for disjoint splitting to check other conflict agents (default inavailable)
                    paths_violate = list()

                    # we do not need to expand the 'disjoint-splitting' is false(disable the disjoint-splitting) node in the constraint tree
                    if constraint['positive'] == True:

                        paths_violate = paths_violate_constraint(constraint,Q['paths'])

                    # based on the selected positive disjoint splitting, check the other conflicted new agents and generate corresponding constraint for each agent, and then assign new path for each agent to satisfy the constraints (only for disjoint splitting available)
                    for new_agent in paths_violate:
                        ...
                        new_path = space_time_a_star(self.my_map, self.starts[new_agent], self.goals[new_agent], self.heuristics[new_agent],new_agent, Q['constraints'])

                        # print("disjoint_valid new path {}".format(new_path))
